...
public class SyncServiceReplicationPolicy extends DefaultReplicationPolicy {
    private static final Logger log = LoggerFactory.getLogger(SyncServiceReplicationPolicy.class);

    public static final String SOURCE_CLUSTER_ALIAS_CONFIG = "source.cluster.alias";
    public static final String REPLICATION_POLICY_TOPIC_MAPPING = "replication.policy.topic.mapping";

    private String sourceClusterAlias = null;
    private Map<String, String> topicMapping = new HashMap<>();
    private Map<String, String> topicMappingReverse = new HashMap<>();
    ...
    public void configure(Map<String, ?> props) {
        ...
        if (props.containsKey(REPLICATION_POLICY_TOPIC_MAPPING)) {
            String mapping = (String) props.get(REPLICATION_POLICY_TOPIC_MAPPING);
            log.info("主题重命名映射配置如下 `{}`.", mapping);

            String[] mappings = StringUtils.split(mapping, ",");
            if (mappings != null && mappings.length > 0) {
                for (String m : mappings) {
                    String[] tm = StringUtils.split(m, ":");
                    if (tm != null && tm.length == 2) {
                        topicMapping.put(tm[0], tm[1]);
                        topicMappingReverse.put(tm[1], tm[0]);
                    }
                }
            }
        }
