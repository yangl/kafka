...
public class ZkOffsetUtils {
    ...
    // 同步标准Kafka消费组的offset
    protected static void syncZkOffsets(CuratorFramework szk, CuratorFramework tzk, OffsetSyncStore offsetSyncStore) {
    ...
    // 同步Jstorm消费组的offset
    protected static void syncJstormOffsets(CuratorFramework szk, CuratorFramework tzk, OffsetSyncStore offsetSyncStore) {
    ...
    // 创建节点
    protected static void create(CuratorFramework cli, String path) {
    ...
    // 检测节点是否存在
    protected static boolean exists(CuratorFramework cli, String path) {
        Stat stat = null;
        if (!Strings.CS.startsWith(path, "/")) {
            path = "/" + path;
        }
